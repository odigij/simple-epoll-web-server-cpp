...
namespace sews::architecture::message::http::dispatch
{
	struct TrieRouter : public core::message::dispatch::Router
	{
		...
		void add(std::string_view method, std::string_view path, core::message::dispatch::Handler *handler) override;
		void remove(std::string_view method, std::string_view path) override;
		core::message::dispatch::Handler *match(const core::message::transport::Message &message) const override;

	  private:
		struct Node
		{
			std::unordered_map<std::string, std::unique_ptr<Node>> children;
			std::unordered_map<std::string, core::message::dispatch::Handler *> handlers;
		};
